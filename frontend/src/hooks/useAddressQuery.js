...
export const useAddress = () => {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: (newAddress) => {
      queryClient.setQueryData(["addresses", user?._id], (old = []) => [
        ...old,
        newAddress,
      ]);
      handleSuccess("Address added successfully");
    },
    onError: (error) => handleError(error?.message || "Failed to Add Address"),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) =>
      handleError(error?.message || "Failed to update address"),
  });
  ...
  const removeMutation = useMutation({
    ...
    onError: (error) =>
      handleError(error?.message || "Failed to remove address"),
  });
  ...
  const markDefaultMutation = useMutation({
    ...
    onError: (error) => handleError(error?.message || "Failed to mark default"),
  });
