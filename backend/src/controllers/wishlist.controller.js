...
const removeFromWishlist = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedWishlist, "Product Removed Successfully"),
    );
});
  ...
  const wishlist = await Wishlist.aggregate([
    ...
    { $unwind: "$productDetails" },
    {
      $addFields: {
        ...
        "products.image": "$productDetails.productImage",
        "products.originalPrice": "$productDetails.originalPrice",
        "products.offeredPrice": "$productDetails.offeredPrice",
        "products.stock": {
          $cond: {
            if: {
