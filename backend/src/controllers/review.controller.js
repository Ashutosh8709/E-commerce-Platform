...
const addReview = asyncHandler(async (req, res) => {
  ...
  if (req.userHasPurchased !== true) {
    throw new ApiError(403, "You can only review purchased products");
  }
  ...
  if (!localReviewImagePath) {
    throw new ApiError(404, "Local file not found for review Images");
  }

  const reviewImage = await uploadOnCloudinary(localReviewImagePath);
  if (!reviewImage) {
    throw new ApiError(400, "Review Image not uploaded on Cloudinary");
  }
  ...
  return res
    .status(200)
    .jsno(new ApiResponse(200, review, "Review Added Successfully"));
});
...
const updateReviewImage = asyncHandler(async (req, res) => {
  ...
  if (!updatedImageLocalPath) {
    throw new ApiError(400, "Local Path for updated image not found");
  }
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, existingReview, "Review Image Updated successfully"),
    );
});
